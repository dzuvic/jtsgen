...
@Value.Immutable
public abstract class TSFunction extends TSType {
    ...
    public TSType changedNamespace(String namespace, List<TSMember> members, List<TSMethod> methods, List<TSConstant> mappedConstants) {
        return TSFunctionBuilder.copyOf(this).withNamespace(namespace).withMembers(members).withMethods(methods).withConstants(mappedConstants);
    }
}
